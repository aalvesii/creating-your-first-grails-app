...
WARNING: If you're using a Database connection like PostgreSQL or MySQL (as we'll see latter on this tutorial) you might need to run the save methods within a transaction. One way of dooing this as follows:
[source,groovy]
Make.withTransaction {
  nissan.save()
  ford.save()
}
